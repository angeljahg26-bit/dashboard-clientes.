...
from core import (
    ...
    compute_kpis_sap, previous_month_column,
)
...
meta_avance_pct = st.sidebar.number_input(
    "🎯 Meta de avance (%)", min_value=1, max_value=100, value=85, step=1,
    help="Se usa para calcular cuántos clientes más necesitan comprar para llegar a la meta.",
)


def clientes_para_meta(total: int, con_venta: int, meta_pct: float) -> int:
    """Cuántos clientes con venta adicionales se necesitan para llegar a la meta."""
    if total <= 0:
        return 0
    faltan = math.ceil(meta_pct / 100 * total) - con_venta
    return max(0, faltan)
...
gap_global = clientes_para_meta(kpis["total_clientes"], kpis["n_con_venta"], meta_avance_pct)
c5.metric(
    f"🎯 Faltan p/ {meta_avance_pct:.0f}%",
    f"{gap_global:,}" if gap_global > 0 else "✅ Meta cumplida",
    help=f"Clientes con venta adicionales necesarios para llegar al {meta_avance_pct:.0f}% de avance.",
)
...
# ---------------------------------------------------------------------------
# 6b. Detalle por ruta: mismas 4 métricas del resumen general, para cada ruta
# ---------------------------------------------------------------------------
st.subheader("📍 Detalle por ruta")
if ruta_status is not None and len(ruta_status):
    col_ruta_name = final_mapping["ruta"]
    sin_venta_full = kpis["sin_venta_detalle"]  # trae _avg_hist antes de limpiar columnas
    for _, row in ruta_status.sort_values(col_ruta_name).iterrows():
        ruta_nombre = row[col_ruta_name]
        total_r = int(row["total"])
        sin_venta_r = int(row["sin_venta"])
        con_venta_r = int(row["con_venta"])
        avance_r = (con_venta_r / total_r * 100) if total_r else 0.0
        riesgo_r = (
            float(sin_venta_full.loc[sin_venta_full[col_ruta_name] == ruta_nombre, "_avg_hist"].sum())
            if "_avg_hist" in sin_venta_full.columns
            else 0.0
        )
        st.markdown(f"**Ruta {ruta_nombre}**")
        rc1, rc2, rc3, rc4, rc5 = st.columns(5)
        rc1.metric("🔴 Sin venta", f"{sin_venta_r:,}")
        rc2.metric("👥 Total cartera", f"{total_r:,}")
        rc3.metric("📈 Avance del día", f"{avance_r:.1f}%")
        rc4.metric("💸 Venta en riesgo", f"${riesgo_r:,.0f}")
        gap_r = clientes_para_meta(total_r, con_venta_r, meta_avance_pct)
        rc5.metric(f"🎯 Faltan p/ {meta_avance_pct:.0f}%", f"{gap_r:,}" if gap_r > 0 else "✅")
else:
    st.info("No hay columna de 'ruta' mapeada.")
...
# ---------------------------------------------------------------------------
# 6c. Clientes sin venta por frecuencia (con detalle al seleccionar una fila)
# ---------------------------------------------------------------------------
st.subheader("📊 Clientes sin venta por frecuencia")
freq_data = kpis["frecuencia_breakdown"]
if freq_data is not None and len(freq_data):
    freq_table = freq_data.reset_index()
    freq_table.columns = ["Frecuencia", "Clientes sin venta"]

    st.caption("👉 Toca una fila para ver el detalle de esos clientes.")
    freq_event = st.dataframe(
        freq_table, use_container_width=True, hide_index=True,
        on_select="rerun", selection_mode="single-row", key="freq_table_select",
    )

    selected_rows = freq_event.selection.rows if freq_event and freq_event.selection else []
    if selected_rows:
        freq_sel = freq_table.iloc[selected_rows[0]]["Frecuencia"]
        col_freq_name = final_mapping["frecuencia"]
        sin_venta_full = kpis["sin_venta_detalle"]
        detalle_freq = sin_venta_full[sin_venta_full[col_freq_name] == freq_sel]
        detalle_freq = detalle_freq.drop(columns=[c for c in detalle_freq.columns if c.startswith("_")], errors="ignore")
        detalle_freq = detalle_freq.drop(columns=[c for c in cols_ocultar if c in detalle_freq.columns], errors="ignore")
        st.markdown(f"**Clientes sin venta con frecuencia {freq_sel}** ({len(detalle_freq)}):")
        st.dataframe(detalle_freq, use_container_width=True, hide_index=True)
else:
    st.info("No hay columna de 'frecuencia' mapeada.")
...
# ---------------------------------------------------------------------------
# 6d. Clientes recuperados (no compraron el mes anterior y sí este mes)
# ---------------------------------------------------------------------------
if sap_mode:
    st.subheader("🟢 Clientes recuperados este mes")
    recuperados = kpis.get("recuperados")
    prev_month_col = kpis.get("prev_month_col")
    if recuperados is not None and len(recuperados):
        st.caption(
            f"No compraron en **{prev_month_col}** y sí tienen venta en **{current_month_col}**."
        )
        cols_recuperados = []
        for key in ("cliente_id", "cliente_nombre", "ruta", "frecuencia"):
            c = final_mapping.get(key)
            if c and c in recuperados.columns:
                cols_recuperados.append(c)
        tabla_recuperados = recuperados[cols_recuperados + ["_venta_mes_anterior", "_venta_mes_actual"]].rename(
            columns={
                "_venta_mes_anterior": f"Venta {prev_month_col}",
                "_venta_mes_actual": f"Venta {current_month_col}",
            }
        )
        tabla_recuperados = tabla_recuperados.drop(
            columns=[c for c in cols_ocultar if c in tabla_recuperados.columns], errors="ignore"
        )
        st.metric("Clientes recuperados", f"{len(tabla_recuperados):,}")
        st.dataframe(
            tabla_recuperados.style.format({
                f"Venta {prev_month_col}": "${:,.0f}", f"Venta {current_month_col}": "${:,.0f}",
            }),
            use_container_width=True, hide_index=True,
        )
    else:
        st.info("No hay clientes recuperados en la selección actual (o no hay un mes anterior con datos).")
    st.divider()
